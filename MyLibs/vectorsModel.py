...
def create_vectors_model(src_path, src_file, win_size, iters = 10, min_count = 10, size = 300, workers = 4, special_tags = []):   
    ...
    vec_model = create_special_tags_vectors(special_tags, vec_model)

    print("End create model vectors at: " , datetime.datetime.now())

    return vec_model
 
def load_vectors_model(model_path, special_tags = []):
    vec_model = gensim.models.Word2Vec.load(model_path)
    vec_model = create_special_tags_vectors(special_tags, vec_model)
    return vec_model

def get_model_vectors(src_path, win_size = 10 ,iters = 10, min_count = 10, size = 300, workers = 4, special_tags = []):
    ...
    if exists(vec_model_path):
        vec_model = load_vectors_model(vec_model_path, special_tags)
    else:
        vec_model = create_vectors_model(src_path, src_file, win_size, iters , min_count, size, workers, special_tags)
...
def create_special_tags_vectors(special_tags ,vec_model):
