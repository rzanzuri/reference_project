...
def triangle_text(my_file, token_file, vector):
    ...
    with open(my_file, encoding="utf-8") as f:
        ...
        print(f"count lines before: {len(orig_lines)}.")
        lines = orig_lines + orig_lines + orig_lines
    ...
    total_miss = 0
    for miss_word in missing_words:
        total_miss += missing_words[miss_word]
        if missing_words[miss_word] > 10000:
            print("The word", miss_word, "is missed", missing_words[miss_word],"times.")
    print("total miss words is:", total_miss)
...
def add_word_to_dict(dict_file):
    with open(dict_file, "a", encoding="utf-8") as f:
        f.write('"לכם","ל","אתם",""')
        f.write('"אני","","אני",""')
        f.write('"בי","ב","אני",""')

def build_english_text_and_dict(my_file):
    print("In build_english_text_and_dict.")
    to_size = 1024*1024*1024*1.5
    my_dict = {}
    new_file = open(my_file.replace(".txt", "_half.txt"), "w", encoding="utf-8")
    with open(my_file, encoding="utf-8") as f:
        lines = f.readlines()
        print("have", len(lines),"lines.")
        print("Finish to read the file.")
        for i, line in enumerate(lines):
            words = re.findall(r'\d+[.,]\d+[.,]\d+|\d+[.,]\d+|\d+|\w+[\"\'\.]\w+|\w+|\S',line)
            print(*words,file=new_file)
            for word in words:
                if word in my_dict:
                    my_dict[word] = my_dict[word][:3] + (my_dict[word][3] + 1,)
                else:
                    my_dict[word] = Tokenizer.split_to_tokens(word) + (1,)
            
            if int(os.path.getsize(new_file.name)) >= to_size:
                new_file.close()
                break
            if i % 200000 == 0:
                Tokenizer.save_tokens(my_file.replace(".txt", "_half_tokenizer.csv"), my_dict, with_count = 1)
        Tokenizer.save_tokens(my_file.replace(".txt", "_half_tokenizer.csv"), my_dict, with_count = 1)

def fix_dictionary(fname):
    suffixes = ["ion", "ful"]
    with open(r'./Data/EnglishWords.txt') as word_file:
        english_words = set(word.strip().lower() for word in word_file)  
    my_dict = Tokenizer.load_tokens(fname,1)
    for word in my_dict:
        stem_word = my_dict[word][1]
        for suffix in suffixes:
            if stem_word.endswith(suffix) and stem_word.replace(suffix,"e") in english_words:
                my_dict[word] = (my_dict[word][0], stem_word.replace(suffix,"e"), suffix , my_dict[word][3])
                break
            elif stem_word.endswith(suffix) and stem_word.replace(suffix,"") in english_words:
                my_dict[word] = (my_dict[word][0], stem_word.replace(suffix,""), suffix + my_dict[word][2] , my_dict[word][3])
                break
        if stem_word == "-PRON-":
            my_dict[word] = (my_dict[word][0], word, my_dict[word][2] , my_dict[word][3])
        if word.lower() == my_dict[word][2]:
            my_dict[word] = (my_dict[word][0], my_dict[word][1], "" , my_dict[word][3])


    Tokenizer.save_tokens(fname, my_dict, 1)
...
if __name__ == "__main__":
    ...
    #run_in_parallel_go_command(3033,1)
    #parse_tzarfati_tagger(r"C:\Users\rzanzuri\Desktop\reference_project\Data\hebrew_data\out", r"C:\Users\rzanzuri\Desktop\reference_project\Data\hebrew_data\full_hebrew.txt")
    #Statistics.check_ave_of_token(r".\Data\english_data\final_eng_text_half_tokenizer.csv")
    #Statistics.checking_tokenizer_of_all_text(r".\Data\hebrew_data\full_hebrew.txt", r".\Data\hebrew_data\heb_dictionaty.csv")
    #triangle_text(r"C:\Users\rzanzuri\Desktop\For_dror\Triangle_text\English\Original\final_eng_text.txt", r"C:\Users\rzanzuri\Desktop\For_dror\Triangle_text\English\Dictionary\final_eng_text_half_tokenizer.csv", create_vector(300,256,70))
    triangle_text(r"C:\Users\rzanzuri\Desktop\For_dror\Triangle_text\Hebrew\Original_text\HebrewText.txt", r"C:\Users\rzanzuri\Desktop\For_dror\Triangle_text\Hebrew\Dictionary\heb_dictionary.csv", create_vector(300,205))
    #add_word_to_dict( r"C:\Users\rzanzuri\Desktop\reference_project\Data\hebrew_data\out\heb_dictionaty.csv")
    ...
    #build_english_text_and_dict(r".\Data\english_data\final_eng_text.txt")
    #fix_dictionary(r".\Data\english_data\final_eng_text_half_tokenizer.csv")
    #Statistics.check_ave_length_word(r".\Data\english_data\final_eng_text_half.txt")
